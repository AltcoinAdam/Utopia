...
const string strMessageMagic = "eUtopium Signed Message:\n";
    ...
    if (fCheckInputs)
    {
        ...
        if (nFees < MIN_RELAY_TX_FEE)
        {
...

            {
                ...
                // At default rate it would take over a month to fill 1UPM
...
static const int64_t nDiffChangeTarget = 5002;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "eutopium", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
FILE* AppendBlockFile(unsigned int& nFileRet)
{
    ...
    while (true)
    {
        ...
        if (fseek(file, 0, SEEK_END) != 0)
            return NULL;
        // FAT32 file size max 4UPM, fseek and ftell max 2UPM, so we must stay under 2UPM
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "eUtopium is here to teach and learn";
        CTransaction txNew;
        txNew.nTime = 1399444495;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x186d38d80bd0fc983502784042f09cf6229a8e1f6033f991a86b10b8ccee9e50"));
        block.print();
